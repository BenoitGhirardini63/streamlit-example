...
class synthese():
    ...
    def Synth_Atelier(): 
        ...
        for elements in requette:
            ...
            if "S_" in table:
                ...
                recup_date = bdd.execute("SELECT Date_fin, Codes_modules, Intitules_codes_modules FROM S_{} WHERE Suivi_module='True' AND Old_categories='Production' AND Date_fin<>''".format(ID_Employee))
                ...
                recup_date = bdd.execute("SELECT Date_fait FROM S_{} WHERE Suivi_module='True' AND Old_categories='Production' AND Date_fait<>''".format(ID_Employee))
                for elem in recup_date:
                    if elem[0] != '':
                        ladate = datetime.strptime(elem[0], '%d-%m-%Y')
                        if ladate > date_of_reactu and ladate < date_limit_of_reactu:
        ...
        for elements in requette:
            ...
            if "S_" in table:
                ...
                try:
                    ...
                    if temp[0] == manager:
                        requette_cat = bdd.execute("SELECT DISTINCT Old_categories FROM S_{}".format(ID_Employee))
                        ...
                        number_of_update, = bdd.execute("SELECT COUNT(Codes_modules) FROM S_{} WHERE Old_categories='Production' AND Suivi_module='True' AND Suivi_categories='True'".format(ID_Employee))
                        recup_date = bdd.execute("SELECT Date_fin, Codes_modules, Intitules_codes_modules FROM S_{} WHERE Suivi_categories='True' AND Suivi_module='True' AND Old_categories='Production' AND Date_fin<>''".format(ID_Employee))
    ...
    def Synth_codes_modules(manager, categories):
        ...
        for elements in requette:
            ...
            if "S_" in table:
                ...
                try:
                    if temp[0] == manager:
                        requette_code = bdd.execute("SELECT Codes_modules, Intitules_codes_modules FROM S_{} WHERE Old_categories='{}' AND Suivi_categories='True'".format(ID_Employee, categories))
    ...
    def Synth_codes_workflow(personne, categories):
        requette_code = bdd.execute("SELECT Codes_modules, Intitules_codes_modules FROM S_{} WHERE Old_categories='{}' AND Suivi_categories='True'".format(personne, categories))
    ...
    def Synth_personne(personne):
        ...
        number_of_update, = bdd.execute("SELECT COUNT(Codes_modules) FROM S_{} WHERE Old_categories='Production' AND Suivi_categories='True'".format(personne))
        retour_info.append(number_of_update[0])

        recup_date = bdd.execute("SELECT Date_fin, Codes_modules, Intitules_codes_modules, Rappel_mail, Date_rappel FROM S_{} WHERE Suivi_categories='True' AND Suivi_module='True' AND Old_categories='Production' AND Date_fin<>''".format(personne))
        ...
        recup_date = bdd.execute("SELECT Date_fait FROM S_{} WHERE Suivi_module='True' AND Old_categories='Production' AND Date_fait<>''".format(personne))
        for elem in recup_date:
            if elem[0] != '':
                ladate = datetime.strptime(elem[0], '%d-%m-%Y')
                if ladate > date_of_reactu and ladate < date_limit_of_reactu:
        ...
        list_categories = bdd.execute("SELECT DISTINCT Old_categories FROM S_{} GROUP BY Old_categories".format(personne))
    ...
    def bilan_cat_personne(chorus, categorie):
        requette = bdd.execute("SELECT Codes_modules, Intitules_codes_modules, Suivi_module, Date_fait, Date_fin FROM S_{} WHERE Old_categories='{}'".format(chorus, categorie))
    ...
    def Synth_charts():
        ...
        for elements in requette:
            ...
            if "S_" in table:
                ...
                requet = bdd.execute("SELECT Manager_name FROM Effectifs WHERE Employee_ID='{}'".format(ID_Employee))
                temp = requet.fetchone()
                number_of_training_by_cat = bdd.execute("SELECT Old_categories, COUNT(Codes_modules) FROM S_{} WHERE Suivi_module='True' GROUP BY Old_categories".format(ID_Employee))
                number_of_training_by_cat_done = bdd.execute("SELECT Old_categories, COUNT(Codes_modules) FROM S_{} WHERE Suivi_module='True' AND Date_fait<>'' GROUP BY Old_categories".format(ID_Employee))
    ...
    def Synth_chart_workflow(personne):
        ...
        number_of_training_by_cat = bdd.execute("SELECT Old_categories, COUNT(Codes_modules) FROM S_{} WHERE Suivi_categories='True' AND Suivi_module='True' GROUP BY Old_categories".format(personne))
        number_of_training_by_cat_done = bdd.execute("SELECT Old_categories, COUNT(Codes_modules) FROM S_{} WHERE Suivi_categories='True' AND Date_fait<>'' GROUP BY Old_categories".format(personne))
    ...
    def recherche_personne_hors_suivi(manager, categories, code):
        ...
        for elem in list_personnes:
            try:
                requet = bdd_cursor.execute("SELECT Suivi_categories FROM S_{} WHERE Date_fait<>'' AND Old_categories='{}' AND Codes_modules='{}'".format(elem[0], categories, code))
                temp = requet.fetchone()
                if temp[0] == 'True':
...
class Workflow():
    def Workflow_choice(personne):
        ...
        if Id:
            ...
            list_categories = bdd.execute("SELECT Old_categories, COUNT(Old_categories) FROM S_{} WHERE Suivi_categories='True' AND Suivi_module='True' AND Date_fait<>'' GROUP BY Old_categories".format(Id[0]))
